...
namespace drawing
{
	...
	void renderer_t::draw_text(const std::string& text, const base::point_t& point) const
	{
		...
		font_->DrawText(sprite_, text.data(), text.length(), &rc, format, 0xFFFFFFFF);
	}
	...
	{
		fps_data_.tick_count = clock() * 0.001f;
		++fps_data_.fps;
		if (fps_data_.tick_count - fps_data_.last_tick_count >= 1.0f)
		{
			fps_data_.last_tick_count = fps_data_.tick_count;
			fps_data_.last_fps = fps_data_.fps;
			fps_data_.fps = 0;
		}
